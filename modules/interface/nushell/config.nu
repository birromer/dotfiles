...
def ros-start [] {
    if not (which docker | is-empty) {
        let args = [
            "run", "--rm", "-it", "--privileged", "--net=host", "--ipc=host", "--env=DISPLAY",
            ...
            "-e", $"DOCKER_USER_ID=(id -u | str trim)",
            "-e", $"DOCKER_USER_GROUP_NAME=(id -gn | str trim)",
            "-e", $"DOCKER_USER_GROUP_ID=(id -g | str trim)",
...
def ros-connect [] {
    if not (which docker | is-empty) {
        let container = (docker ps -aq --filter ancestor=birromer/ros-noetic-intervals:latest --filter status=running | str trim)
        if not ($container | is-empty) {
...
def ros-clean [] {
    if not (which docker | is-empty) {
        let containers = (docker ps -aq --filter ancestor=birromer/ros-noetic-intervals:latest --filter status=exited | str trim)
        if not ($containers | is-empty) {
            docker rm $containers
        } else {
            echo "No exited ROS containers to clean"
        }
    }
}
